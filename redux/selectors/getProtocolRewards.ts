...
export const getProtocolRewardsMEME = createSelector(
...
export const getTokenNetBalanceRewardsMEME = createSelector(
  (state: RootState) => state.assetsMEME,
  (assets) => {
    const tokenNetBalanceRewards = Object.entries(assets.allFarms?.tokenNetBalance || {}).reduce(
      (acc, cur) => {
...
export const getNetLiquidityRewardsMEME = createSelector(
  (state: RootState) => state.assetsMEME,
  (assets) => {
    const rewards = Object.entries(assets.netTvlFarm).map(
      ([tokenId, farm]: [string, INetTvlFarmReward]) => {
